...
const handleSearchResults = async (e)=>{
   ...
   try{
    ...
    const response=await fetch(`http://localhost:5001/api/data/sector/${searchedText}`);
    const data= await response.json();
    if(data.success===true) setSectorData(data.data);
  ...
  return (
    ...
    className='w-3/4 ml-10 '>
       ...
       type='text' 
       placeholder='Search the result according to sector'/>
      ...
      <div className='flex flex-wrap overflow-y-scroll h-screen'>
       ...
       { sectorData.length>0 ? sectorData.map((item)=><DataCard key={item.name} item={item}/>) : 
