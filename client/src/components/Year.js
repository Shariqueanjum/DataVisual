...
const handleSearchResults = async (e)=>{
   ...
   try{
    ...
    const response=await fetch(`http://localhost:5001/api/data/year/${searchedText}`);
    const data= await response.json();
    if(data.success===true) setYearData(data.data);
  ...
  return (
    ...
    className='w-3/4 ml-10 '>
       ...
       type='text' 
       placeholder='Search the result according to year'/>
      ...
      <div className='flex flex-wrap overflow-y-scroll h-screen'>
       ...
       { yearData.length>0 ? yearData.map((item)=><DataCard key={item.name} item={item}/>) : 
