...
const Country = () => {
  ...
  const handleSearchResults = async (e)=>{
     ...
     try{
      ...
      const response=await fetch(`http://localhost:5001/api/data/country/${searchedText}`);
      const data= await response.json();
      if(data.success===true) setCountryData(data.data);
  ...
  return (
    ...
    className='w-3/4 ml-10 '>
       ...
       type='text' 
       placeholder='Search the result according to country'/>
      ...
      <div className='flex flex-wrap overflow-y-scroll h-screen'>
       ...
       { countryData.length>0 ? countryData.map((item)=><DataCard key={item.name} item={item}/>) : 
