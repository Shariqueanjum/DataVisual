...
const Likelihood = () => {
    ...
    const handleSearchResults = async (e)=>{
       ...
       try{
        ...
        const response=await fetch(`http://localhost:5001/api/data/likelihood/${searchedText}`);
        const data= await response.json();
        if(data.success===true) setLikelihoodData(data.data);
  ...
  return (
    ...
    className='w-3/4 ml-10 '>
       ...
       type='text' 
       placeholder='Search the result according to likelihood'/>
      ...
      <div className='flex flex-wrap overflow-y-scroll h-screen'>
       ...
       { likelihoodData.length>0 ? likelihoodData.map((item)=><DataCard key={item.name} item={item}/>) : 
