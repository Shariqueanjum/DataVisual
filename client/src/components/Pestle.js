...
const handleSearchResults = async (e)=>{
   ...
   try{
    ...
    const response=await fetch(`http://localhost:5001/api/data/pestle/${searchedText}`);
    const data= await response.json();
    if(data.success===true) setPestleData(data.data);
...
return(
  ...
  className='w-3/4 ml-10 '>
     ...
     type='text' 
     placeholder='Search the result according to pestle'/>
    ...
    <div className='flex flex-wrap overflow-y-scroll h-screen'>
     ...
     { pestleData.length>0 ? pestleData.map((item)=><DataCard key={item.name} item={item}/>) : 
