...
export const filteredByTopic = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ topic: { $regex: topic, $options: 'i' } });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
...
export const filteredBySector = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ sector: { $regex: sector, $options: 'i' } });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
...
export const filteredByRegion = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ region: { $regex: region, $options: 'i' } });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
...
export const filteredByCountry = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ country: { $regex: country, $options: 'i' } });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
...
export const filteredByPestle = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ pestle: { $regex: pestle, $options: 'i' } });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
...
export const filteredByIntensity = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ intensity: parseInt(intensity) });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
...
export const filteredByLikelihood = async (req, res) => {
    try {
        ...
        const allData = await Report.find({ likelihood: parseInt(likelihood) });
        if (!allData || allData.length === 0) {
            return res.status(400).json({
                success: false,
                message: "No Data Found",
